...
class ManageTeamsBloc extends Bloc<ManageTeamsEvent, ManageTeamsState> {
  ManageTeamsBloc({required TeamsRepository teamsRepository, required GameRepository gameRepository})
  ...
  Future<void> _onLoadRequested(ManageTeamsLoadRequested event, Emitter<ManageTeamsState> emit) async {
    ...
    try {
      ...
      final availableUsers = allUsers.where((u) => !teamMemberIds.contains(u.id)).toList();

      emit(ManageTeamsLoaded(teamId: event.teamId, gameId: event.gameId, teamName: team.name, teamSize: game.teamSize, teamMembers: teamMembers, availableUsers: availableUsers));
    } catch (e) {
  ...
  Future<void> _onAddMember(ManageTeamsAddMember event, Emitter<ManageTeamsState> emit) async {
    if (state.teamId == null || state.gameId == null || state.teamName == null || state.teamSize == null) return;

    // Check if team is full
    if (state.teamMembers.length >= state.teamSize!) {
      emit(ManageTeamsError('Team is full (${state.teamSize} members max)'));
      return;
    }
